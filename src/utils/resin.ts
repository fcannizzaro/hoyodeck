 ...
 *   4. Bottom text pill with current resin count
 *
...
export const buildResinSvg = (
  ...
  <image href="${resinDataUri}" x="${float.x}" y="${float.y}" width="${SIZE}" height="${SIZE}" preserveAspectRatio="xMidYMid meet" transform="rotate(${float.r}, ${CENTER + float.x}, ${CENTER + float.y})" />
  <rect x="0" y="0" width="${SIZE}" height="${coverH}" fill="black" opacity="0.6" />
  <rect x="${barX}" y="${BAR_Y}" width="${barW}" height="${BAR_HEIGHT + BAR_RX}" rx="${BAR_RX}" fill="black" opacity="0.5" />
