...
export function StatusMessage({ message, type, duration = 3000, onDismiss }: StatusMessageProps) {
  useEffect(() => {
  ...
  return <div className={`p-2 rounded text-[11px] border ${TYPE_STYLES[type]}`}>{message}</div>;
}
