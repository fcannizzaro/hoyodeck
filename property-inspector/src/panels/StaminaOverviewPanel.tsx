...
import type { GameId, HoyoAccountInfo, StaminaSlot } from "@hoyodeck/shared/types";
...
interface SlotRowProps {
  ...
  onUpdate: (index: number, slot: StaminaSlot) => void;
...
export function StaminaOverviewPanel() {
  ...
  const slots: StaminaSlot[] = (settings.slots as StaminaSlot[] | undefined) ?? [];

  const updateSlot = (index: number, updated: StaminaSlot) => {
    const next = [...slots];
    next[index] = updated;
  ...
  return (
    <div className="flex flex-col gap-2">
      <Heading>Stamina Overview</Heading>

      {slots.length === 0 && (
        <p className="text-[11px] text-sd-secondary">
          Add game slots to display stamina on the encoder.
        </p>
      )}
